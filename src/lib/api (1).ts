...
export async function fetchFolderPhotos(folderLink: string): Promise<DrivePhoto[]> {
  ...
  // Entries created before the OneDrive migration still have their photos
  // in Google Drive — tell the backend which source to read from so those
  // older entries keep working instead of coming back empty.
  const source = folderLink.includes("#oid=") ? "onedrive" : "drive";

  const res = await fetchWithRetry(
    `${APPS_SCRIPT_URL}?action=photos&folderId=${folderId}&source=${source}`
  );
